...
def find_app_path(app_name):
    # List of common locations to search for apps
    search_locations = [
        "C:\\Program Files",
        "C:\\Program Files (x86)",
        "C:\\Windows",
    ...
    extensions = [".exe", ".bat", ".cmd", ".lnk", ".pdf", ".mp4", ".mp3", ".docx", ".pdf", ".png", ".jpg", ".xlxs", ".pptx", ".ppt", ".txt"]
    
    # Convert app name to lowercase and handle Tamil app names
    app_translations = {
        "நோட்": "notepad",
        "குரோம்": "chrome",
        "கடிகாரம்": "clock",
        "கணக்குப்பெறி": "calculator"
    }
    
    # Translate Tamil app names to English if needed
    if app_name in app_translations:
        app_name = app_translations[app_name]
    
    # Search for the app in different locations
    for location in search_locations:
        for root, dirs, files in os.walk(location):
            for filename in files:
                # Check if filename matches app name with various extensions
                if any(app_name.lower() in filename.lower() and ext in filename.lower() for ext in extensions):
                    return os.path.join(root, filename)
    
    # Special handling for some known apps
    special_apps = {
    ...
    if app_name.lower() in special_apps:
        return special_apps[app_name.lower()]
    ...
    if "change to dark mode" in lower_input:
        ...
        if "app" in lower_input or "ஆப்" in lower_input:  # English and Tamil "app" keywords
            ...
            if app_path:
                ...
                return f"App '{app_name}' not found." if "app" in lower_input else f"ஆப் '{app_name}' கிடைக்கவில்லை."
